...
// The digest of one TDH.MEM.PAGE.ADD per page and one TDH.MR.EXTEND per chunk
// of it. A page the loader fills is added without being extended, so only the
// address it was added at reaches MRTD.
const MR_RECORD_LEN: usize = 128;
const MR_GPA: usize = 16;
const MR_EXTEND_CHUNK: usize = 256;

pub fn calculate(pages: &[Page]) -> [u8; 48] {
    let mut hash = Sha384::new();
    for page in pages {
        let mut add = [0u8; MR_RECORD_LEN];
        add[..12].copy_from_slice(b"MEM.PAGE.ADD");
        add[MR_GPA..MR_GPA + 8].copy_from_slice(&page.gpa.to_le_bytes());
        hash.update(add);
        ...
        for offset in (0..PAGE as usize).step_by(MR_EXTEND_CHUNK) {
            ...
            extend[MR_GPA..MR_GPA + 8].copy_from_slice(&(page.gpa + offset as u64).to_le_bytes());
            hash.update(extend);
