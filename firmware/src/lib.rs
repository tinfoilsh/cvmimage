...
pub struct Launch {
    ...
    /// Kept because the SEV-SNP image republishes it as required memory.
    pub e820: Vec<(u64, u64, u32)>,
...
pub fn tdx(kernel_path: &Path, initramfs_path: &Path, params: &Params) -> Result<Launch, String> {
    ...
    let zero = boot::zero_page(&p.setup, p.info, initramfs_len, ACPI_BASE, &e820, 0)?;
