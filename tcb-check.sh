...
require_file_contains "$repo_dir/tinfoil/cmd/initrd/main.go" 'return initrdModuleBuiltinMode, nil' "compiled initrd defaults to built-in dm targets"
...
require_file_contains "$repo_dir/tinfoil/cmd/init/main.go" 'ctx, cancel := bootContext\(parent\)' "Tinfoil PID1 routes boot deadline through an explicit selector"
require_file_contains "$repo_dir/tinfoil/cmd/init/main.go" 'runOneShotHardened\(ctx, tinfoilBootTimeout\(\), "tinfoil-boot"' "Tinfoil PID1 starts boot under no_new_privs policy"
...
require_file_contains "$repo_dir/tinfoil/cmd/boot/network_ready.go" '/usr/sbin/ip", "link", "set", "dev"' "Tinfoil activates kernel Ethernet links without udev"
...
require_file_contains "$repo_dir/tinfoil/cmd/boot/network_ready.go" 'ip", "addr", "replace"' "Tinfoil applies DHCPv4 addresses explicitly"
require_file_contains "$repo_dir/tinfoil/cmd/boot/network_ready.go" 'ip", "route", "replace", "default"' "Tinfoil applies the DHCPv4 default route explicitly"
...
require_file_contains "$repo_dir/tinfoil/cmd/init/main.go" 'startOptionalNVIDIA\(ctx\)' "Tinfoil PID1 owns optional NVIDIA startup"
