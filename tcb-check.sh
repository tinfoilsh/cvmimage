...
require_file_contains "$repo_dir/tinfoil/cmd/initrd/main.go" 'func readVeritySalt' "compiled initrd reads only the salt from the host-controlled verity superblock"
require_file_contains "$repo_dir/tinfoil/cmd/initrd/main.go" 'func verityDataBlocks' "compiled initrd derives verity data-block count from sysfs, not the superblock"
require_file_not_contains "$repo_dir/tinfoil/cmd/initrd/main.go" 'func parseVeritySuperblock' "compiled initrd no longer parses the full host-controlled verity superblock"
require_file_contains "$repo_dir/tinfoil/cmd/initrd/main.go" 'verityDataBlockSize  = 4096' "compiled initrd pins the verity data block size as a build invariant"
require_file_contains "$repo_dir/tinfoil/cmd/initrd/main.go" 'verityHashAlgorithm  = "sha256"' "compiled initrd pins the verity hash algorithm as a build invariant"
require_file_contains "$repo_dir/mkosi.repart/10-root.conf" '^Verity=data$' "root data partition uses dm-verity (matches pinned initrd params)"
require_file_contains "$repo_dir/mkosi.repart/11-root-verity.conf" '^Verity=hash$' "root hash partition uses dm-verity (matches pinned initrd params)"
