...
const (
	...
	Issuer     = "https://orchestrator.tinfoil.sh"
	PublicKey  = "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEe7Dz4UoNi9fKoAq9DMgv+moqp+dyZyVSeRlAiVlFqZNAMLaamYJotUmKSR7OAT7AND1fC47oDfzOOX9nOs72ew=="
	coordinate = 32
)

// Check verifies the signature before checking the issuer, domain, expiry, and boot nonce.
func Check(token, domain, nonce, volume string) error {
	key, err := ParseKey(PublicKey)
	if err != nil {
		return err
	}
	return CheckWithKey(key, token, domain, nonce, volume)
}

func CheckWithKey(key *ecdsa.PublicKey, token, domain, nonce, volume string) error {
	if key == nil || key.Curve != elliptic.P256() || domain == "" || nonce == "" {
		return errors.New("invalid permit authority")
	}
	...
	switch {
	case volume != "" && (claims.Volume != volume || claims.Action != "volume-unlock"):
		return errors.New("permit does not authorize this volume")
	case claims.Issuer != Issuer:
		return fmt.Errorf("token issuer is %q, not %q", claims.Issuer, Issuer)
	case claims.Subject != domain:
		return fmt.Errorf("token names %q, not this guest", claims.Subject)
	case claims.Expires == 0:
		return errors.New("token does not expire")
	case time.Now().After(time.Unix(claims.Expires, 0)):
		return errors.New("token has expired")
	case !slices.Contains(audiences(claims.Audience), nonce):
		return errors.New("token is bound to another boot")
	}
	return nil
}

func ParseKey(encoded string) (*ecdsa.PublicKey, error) {
