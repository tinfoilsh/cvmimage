...
func testSandbox(t *testing.T) (*testAuthority, *ecdsa.PrivateKey) {
	t.Helper()
	key, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	return &testAuthority{domain: "sandbox.example", nonce: "test-boot", permit: &key.PublicKey}, key
}

func testClaims() map[string]any {
	return map[string]any{
		"iss": Issuer, "sub": "sandbox.example", "aud": "test-boot",
...
func (s *testAuthority) check(token string) error {
	return CheckWithKey(s.permit, token, s.domain, s.nonce, "")
}
func TestOperatorScope(t *testing.T) {
	box, key := testSandbox(t)
	claims := testClaims()
	token := signPermit(t, key, "ES256", claims)
	if err := CheckWithKey(box.permit, token, box.domain, box.nonce, "state"); err == nil {
		t.Fatal("owner permit accepted as operator permit")
	}
	claims["volume"] = "state"
	claims["action"] = "volume-unlock"
	token = signPermit(t, key, "ES256", claims)
	if err := CheckWithKey(box.permit, token, box.domain, box.nonce, "state"); err != nil {
		t.Fatal(err)
	}
	if err := CheckWithKey(box.permit, token, box.domain, box.nonce, "other"); err == nil {
		t.Fatal("permit accepted for another volume")
	}
	if err := CheckWithKey(box.permit, token, box.domain, "another-boot", "state"); err == nil {
		t.Fatal("permit accepted after reboot")
	}
}
